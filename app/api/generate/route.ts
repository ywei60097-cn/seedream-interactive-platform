...
export async function POST(request: Request) {
  try {
    ...
    const { prompt, image: submittedImage, source, marked, coordinateTokens = [], markInstructions = "", interactionMode = "mark", ratio = "1:1", outputWidth = 1024, outputHeight = 1024, imageArea = false, resolution = "1K", advanced = {} } = clientPayload; const debug = clientPayload.debug === true;
    ...
    const enhancedPrompt = isCoordinateMode
      ? `请根据图1完成精准图片编辑。${coordinateInstruction}${markInstruction}用户要求：${prompt}。${ratioInstruction}仅修改坐标所指区域；保持构图、人物身份和未指定区域与原图一致，使结果自然融合。`
      : `请根据图1中的手绘草图、涂鸦、圈选、箭头或标记区域进行图片编辑。${markInstruction}用户要求：${prompt}。${ratioInstruction}仅修改标记所指区域；移除所有草图和标记线条；保持构图、人物身份和未标记区域与原图一致，使结果自然融合。`;
    ...
    const requestBody: Record<string, unknown> = { model, prompt: enhancedPrompt, image: [inputImage], size, output_format: outputFormat, response_format: "b64_json", watermark: Boolean(advanced.watermark), optimize_prompt_options: { mode: "standard" } };
