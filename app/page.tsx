...
type UploadedSource = { id: string; name: string; image: string };
...
export default function Home() {
  ...
  const activeSource = uploadedSources.find(item => item.id === activeSourceId);
  ...
  const replaceActiveSource = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return;
    const replacement = await readSourceFile(file);
    const targetId = activeSourceId || uploadedSources[0]?.id;
    if (!targetId) { setUploadedSources([replacement]); selectSourceImage(replacement); return; }
    const next = { ...replacement, id: targetId };
    setUploadedSources(current => current.map(item => item.id === targetId ? next : item));
    selectSourceImage(next);
  };
  const removeSourceImage = (id: string) => {
    const remaining = uploadedSources.filter(item => item.id !== id);
    setUploadedSources(remaining);
    if (id !== activeSourceId) return;
    const next = remaining[0];
    if (next) selectSourceImage(next); else clearCurrentImage();
  };
  ...
  const hasEditInstruction = Boolean(prompt.trim() || markInstructions);
  const optimizePrompt = () => {
    const target = prompt.trim() || markInstructions || (workspace === "layers" ? "画面中的主要主体、文字与前景元素" : "标记所指区域");
    const optimized = workspace === "layers"
      ? `将图1拆分为可单独编辑的图层。请分离：${target}。多个对象必须分别输出为独立的透明图层，同时输出一张完整背景底图；保持每个对象的位置、比例、边缘和文字完整清晰，不要合并主体与文字。`
      : `根据图1完成精准图片编辑。编辑目标：${target}。${markInstructions ? `请逐项执行以下标记意图：${markInstructions}。` : ""}仅修改标记所指区域，移除所有辅助标记线，并保持未标记区域的构图、主体身份和细节与原图一致。`;
    setPrompt(optimized);
    showToast(workspace === "layers" ? "已优化为多对象图层分离指令。" : "已优化为精准交互编辑指令。");
  };
  const editSelectedLayer = () => { if (!selectedLayer) return; setActiveSourceId(""); setSource(selectedLayer.image); setResult(""); setShowResult(false); setWorkspace("edit"); clearEditingState(); };
  const clearCurrentImage = () => { resetLayerResults(); setUploadedSources([]); setActiveSourceId(""); setSource(""); setResult(""); setShowResult(false); setPrompt(""); setError(""); setWorkspace("edit"); setTool("point"); clearEditingState(); };
  ...
  const generate = async () => {
    const effectivePrompt = prompt.trim() || (markInstructions ? "请按照各标记意图完成精准编辑。" : "");
    if (!canvasSource || !effectivePrompt) { setError("请填写编辑指令，或为至少一个标记填写编辑意图。"); return; } setBusy(true); setError("");
    const referenceImages = uploadedSources.filter(item => item.id !== activeSourceId).map(item => item.image);
    const clientRequest = { prompt: effectivePrompt, ratio, outputWidth, outputHeight, imageArea, resolution, advanced, interactionMode, image: interactionMode === "coordinate" ? canvasSource : exportMarkedImage(), referenceImages, coordinateTokens: interactionMode === "coordinate" ? buildCoordinateTokens() : [], markInstructions, debug: debugEnabled };
